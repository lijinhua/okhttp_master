...
public final class FormBody extends RequestBody {
   ...
   * 该方法有两个功能，一个是将内容写到bufferedsink，二是统计内容的长度，通过返回值返回
   */
  private long writeOrCountBytes(@Nullable BufferedSink sink, boolean countBytes) {
    long byteCount = 0L;

    Buffer buffer;
    // 如果统计内容长度，重新创建一个buffer
    if (countBytes) {
      buffer = new Buffer();
    } else {
      buffer = sink.buffer();
  ...
  public static final class Builder {
    // 定义了两个自己集合，用来封装我们传递进来的key，value，而我们使用的httpclient在传递表单参数时，却使用了BasicNameValuePair，
    // 他将key,value包封装成了一个BasicNameValuePair，然后就使用一个集合就可以了，这样使用两个开销更大
